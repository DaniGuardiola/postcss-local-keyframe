...
module.exports = function (statement) {
  ...
  if (timings) {
    ...
    remainder = timings.reduce(
      (remainder, cur) => remainder.replace(cur, ""),
      remainder
    );
  }
  ...
  const timingRegexp = new RegExp(
    "(" + timingFn.join("(\\s|$)|") + "|cubic-bezier\\(.*?\\))"
  );
  ...
  /* -- Get direction -- */
  const direction = statement.match(
    /(normal|alternate|alternate-reverse|reverse)\s*/
  );
